...
function Login() {
  ...
  const numericOnly = (phone) => {
    ...
    if (
     phone === "" ||
      (regNumeric.test(phone) && phone.length <= 12)
    ) {
  ...
  const completeLogin = async () => {
    ...
    try {
      ...
      // const { data } = ;
      //console.log(data);
      if (error?.response?.data?.message === "login/invalid-phone-or-password") {
      ...
      {isLoginComponent ? (
        <>
          ...
          <p className={classes.para}>
            By continuing, you agree to ShopSavvy's Terms of Use and Privacy
            Policy.
          </p>
          <Button
            variant="contained"
          ...
          {/* <Button
            ...
            style={{ background: "#fff", color: "#0802A3" }}
            color="primary"
          ...
          >
            New to ShopSavvy? Create an account
          </a>
        </>
      ) : (
        <OTPVerify
